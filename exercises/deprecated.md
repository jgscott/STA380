...
Note 2: I want your report to be fully reproducible.  Of course, it would seem that, by its very nature, one thing that cannot be reproduced exactly is a Monte Carlo simulation.  That's OK --- you can try figuring out how to set a seed for your simulation, or you can just accept that it will be a little bit different next time the script is compiled.  
...
Note: this is intended mainly as an exercise in visual and numerical story-telling. Tell your story primarily in plots, and while you can run a regression model if you want, that's not the end goal here.  Telling a story is.  Keep it concise.   
...
In this problem, you will construct sector-based portfolio of exchange-traded funds, or ETFs, and use bootstrap resampling to analyze the short-term tail risk of your portfolio.  If you're unfamiliar with exchange-traded funds, you can read a bit about them [here](http://www.investopedia.com/terms/e/etf.asp).


### The goal  

Suppose you have $100,000 in capital.  Your task is to:  
- Construct three different ETF portfolios, each involving an allocation of your $100,000 in capital to somewhere between 4 and 10 different ETFs.  You can find a [big database of ETFs here.](https://etfdb.com/etfdb-categories/)  Make sure the portfolios are different from each other!  (Maybe one sounds safe, another aggressive, another very diverse, etc...)  You're not being graded on what specific portfolios you choose... just provide some context for your choices.   
...
Note: if you're unfamiliar with value at risk (VaR), see the class website for links to a few different sites that explain the idea.  
...
In the C50train directory, you have ~50 articles from each of 50 different authors (one author per directory).  Use this training data (and this data alone) to build the model.  Then apply your model to the articles by the same authors in the C50test directory, which is about the same size as the training set.  _Describe your data pre-processing and analysis pipeline in detail._  
...
This question will be graded according to two criteria:    
  1. the clarity of your description (i.e. could you analysis be reproduced by a competent data science based on what you've said), and
